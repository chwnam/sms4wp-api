...
HTTP content로 전달되는 JSON에는 최소한 하나의 오브젝트가 포함되어 있습니다. 보통 이 오브젝트는 'detail'이란 이름의 key를 포함하고 있으며 이 키의 값은 서버에서 전달하는 보다 자세한 메시지입니다. 이외의 카-값들은 API의 종류에 따라 상이하며 아래 API 정의에서 설명합니다.
...
인증 토큰을 발행하는 API. scope 값이 이미 기록되어 있는 경우 기존의 인증 토큰이 삭제되고 새로운 토큰으로 갱신된다.
  ...
  - 경로: ``/auth_token/grant/``
  - method: POST
    - membership_id: 유저가 속한 사이트의 유저 id (pk).
    - membership_type_name: 유저가 속한 사이트의 이름.
...
인증된 토큰을 회수하는 API. Revoke 된 이후에는 해당 인증 토큰을 사용할 수 없다. 그러나 데이터베이스 상에서 해당 토큰의 레코드가 삭제되는 것은 아니다.
  ...
  - method: GET
    ...
    - membership_type_name: 유저가 속한 사이트의 이름.
    - scope:    필수. 인증 토큰을 구분하는 태그의 개념이다. 이 파라미터의 값으로는 255자 미만의 문자열을 받는다.
                이 문자열은 모든 데이터베이스 상에서 유일해야 하므로, 플러그인이 설치될 해당 사이트의 도메인으로 정할 
#### Removing ####
...
만일 토큰이 삭제되면 토큰과 관련된 모든 메시지 내역과 결과 자료가 사라질 수 있으니 사용에는 주의해야 한다.
  ...
  - method: DELETE
    - auth_token_id: 필수. 삭제하고픈 auth_token id
...
현재 인증 토큰의 정보를 조회하는 API입니다.
  ...
  - 메소드: GET
  - 파라미터
    - 일반사용자는 파라미터를 필요로 하지 않는다.
    - 관리자의 경우 다음과 같은 파라미터를 선택적으로 입력할 수 있다.
      - membership_id:    선택. 해당 사이트의 유저 pk를 입력한다. 콤마(,)로 복수의 아이디를 입력할 수 있다. membership_type 없이 단독적으로는 사용할 수 없다. 그렇지 않으면 BAD_REQUEST를 리턴한다.
      - membership_type:  선택. 유저가 속한 사이트의 이름. 
      - user_id:          선택. 백엔드 유저 id를 입력 가능하다. membership_id와 같이 사용할 수 없다. 콤마로 복수의 아이디를 입력할 수 있다.
  - 결과
    - id: 현재 토큰의 아이디.
    - user_id: 토큰의 사용자 아이디.
    - value: 토큰 값.
...
이 작업은 관리자의 인증 토큰만이 가능한다.
  ...
  - method: POST
  - parameters:
    - membership_type_name:  필수. 백엔드에 등록된 프론트 엔드 사이트 명.
    - membership_id:    필수. 변경을 원하는 프론트 엔드 유저 primary key.
    - email:            선택. 변경을 원하는 이메일 주소. 생략할 경우 변경하지 않음.
    - group_name        선택. 해당 유저의 그룹. 변경을 원하는 값을 지정. 이를테면, 'admin', 'user'.
...
이 작업은 관리자의 인증 토큰만이 가능한다.
  ...
  - parameters:
    - membership_id: 필수. 변경을 원하는 프론트 엔드 유저 primary key.
    - membership_type_name: 필수. 교체하고 싶은 프론트 엔드 사이트 명.
...
#### Modify user Point ####
  ...
  - parameters:
    - membership_id:
    - membership_type_name:
    - value:           필수. 증감하고픈 포인트. 음수의 경우 차감. 양수의 경우 증가.
...
#### Query User Point ####
  ...
  - parameters:
    ...
    - membership_id:   관리자의 경우에는 특정 회원의 아이디를 입력할 수 있다. 복수의 아이디를 콤마(,)로 구분하여 연속적으로 입력할 수 있다.
    - membership_type: 프론트엔트 타입.
    - user_id: 혹은 백엔드 자체의 id를 입력하여 검색 가능하다. 단, membership_id와 user_id는 같이 사용할 수 없다. 복수의 아이디를 콤마(,)로 구분하여 연속적으로 입력할 수 있다.
  - returns:
    - user_id: 유저의 아이디.
    - value: 유저의 포인트.
...
#### Querying Transaction List ####
  - method: GET
  - parameters:
    - user_id: 옵션. 해당 사용자의 포인트 사용 내역을 출력한다.
    - 관리자의 경우에만 user_id를 필요로 하며, 일반 유저의 경우는 파라미터를 필요로 하지 않는다.
